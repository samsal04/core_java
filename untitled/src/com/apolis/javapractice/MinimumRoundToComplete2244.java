package com.apolis.javapractice;
//You are given a 0-indexed integer array tasks, where tasks[i] represents the difficulty level of a task.
//In each round, you can complete either 2 or 3 tasks of the same difficulty level.
//
//Return the minimum rounds required to complete all the tasks, or -1 if it is not possible to complete all the tasks.
//
//        Example 1:
//        Input: tasks = [2,2,3,3,2,4,4,4,4,4]
//        Output: 4
//        Explanation: To complete all the tasks, a possible plan is:
//        - In the first round, you complete 3 tasks of difficulty level 2.
//        - In the second round, you complete 2 tasks of difficulty level 3.
//        - In the third round, you complete 3 tasks of difficulty level 4.
//        - In the fourth round, you complete 2 tasks of difficulty level 4.
//        It can be shown that all the tasks cannot be completed in fewer than 4 rounds, so the answer is 4.
//
//        Example 2:
//        Input: tasks = [2,3,3]
//        Output: -1
//        Explanation: There is only 1 task of difficulty level 2, but in each round,
//        you can only complete either 2 or 3 tasks of the same difficulty level.
//        Hence, you cannot complete all the tasks, and the answer is -1.

import java.util.Arrays;
import java.util.HashMap;
import java.util.Iterator;
import java.util.Map;

public class MinimumRoundToComplete2244 {
    private static int minimumRounds(int[] tasks) {
        int result=0;
//        tasks= Arrays.stream(tasks).sorted().toArray();
//        System.out.println(Arrays.toString(tasks));

        HashMap<Integer,Integer> map= new HashMap<>();

        for(int i=0;i<tasks.length;i++){
            map.compute(tasks[i],(key,value)-> ( value==null)? 1:value+1);
        }
//        System.out.println(map);
        Iterator iterator= map.entrySet().iterator();

        while (iterator.hasNext()){
            Map.Entry<Integer,Integer> mapIterator= (Map.Entry<Integer, Integer>) iterator.next();

            if(mapIterator.getValue()<=1){
                return -1;
            }
            else {
                result = result + checkValue(mapIterator.getValue());
            }
        }

        return result;
    }

    private static int checkValue(Integer value) {
        int sum=0;
        int reminder=0;
        int division=0;
//
//       if(value<=1){
//           return -1;
//       }
//       if(value>3 && value%3==0){
//           return value/3;
//       }
//       else if (value>3 && value){
//
//       }
        System.out.println(value);
        while (value!=0){

            if(value>=3){
              reminder= value%3;
              division=value/3;
              value=reminder;
              sum= sum+division;

            }
            else {
                reminder= value%2;
                division=value/2;
                value=reminder;
                sum= sum+division;
            }

        }

        System.out.println(sum);
        System.out.println("======================");
        return sum;
    }

    public static void main(String[] args) {

        int[] tasks={2,3,3};
        System.out.println(minimumRounds(tasks));
    }
}
